...
function getMonthlyReport($year, $month) {
    ...
    foreach ($transactions as $transaction) {
        $transaction_date = date('Y-m-d', strtotime($transaction['order_date']));
        $transaction_year = date('Y', strtotime($transaction['order_date']));
        $transaction_month = date('m', strtotime($transaction['order_date']));
        $transaction_day = date('d', strtotime($transaction['order_date']));
        ...
        if ($transaction_year == $year && $transaction_month == $month) {
            ...
            $report['daily_data'][(int)$transaction_day]['transactions']++;
            $report['daily_data'][(int)$transaction_day]['revenue'] += $transaction['total_amount'];
            $report['daily_data'][(int)$transaction_day]['food'] += $transaction['food_count'] ?? 0;
            $report['daily_data'][(int)$transaction_day]['drinks'] += $transaction['drink_count'] ?? 0;
        }
    }
                        ...
                        <table class="table">
                            ...
                            <tbody>
                                ...
                                <tr>
                                    ...
                                    <td><?php echo htmlspecialchars($transaction['customer_kode']); ?></td>
    ...
    <script>
        ...
        const revenueChart = new Chart(revenueCtx, {
            ...
            data: {
                ...
                datasets: [{
                    label: 'Pendapatan (Rp)',
                    data: [150000, 250000, 350000, 500000, 420000, 380000, 450000, 300000, 180000],
                    borderColor: '#36b37e',
        ...
        const paymentChart = new Chart(paymentCtx, {
            type: 'pie',
            data: {
                labels: ['Tunai', 'QRIS', 'Kartu Debit', 'Kartu Kredit', 'E-Wallet', 'Khusus'],
                datasets: [{
                    data: [
                        ...
                        <?php echo $payment_stats['special']; ?>
                    ],
                    backgroundColor: [
                        '#36b37e',
                        '#4d96ff',
