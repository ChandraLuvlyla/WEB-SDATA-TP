...
if (!$order) {
    // Cek apakah ini pending order
    $requests = getJSONData(ORDER_REQUESTS_FILE);
    foreach ($requests as $request) {
        if (isset($request['order_number']) && $request['order_number'] == $order_number) {
            $order = $request;
...
$customer_kode = $order['customer_kode'] ?? '-';
    ...
    <div class="receipt">
        ...
        <div class="customer-info">
            ...
            <div class="info-row">
                ...
                <span>
                    <?php 
                    $date_field = isset($order['order_date']) ? $order['order_date'] : $order['request_date'] ?? date('Y-m-d H:i:s');
            ...
            <div class="info-row">
                ...
                <span><?php echo htmlspecialchars($customer_kode); ?></span>
            </div>
        </div>
        ...
        <div class="summary">
            ...
            <?php if ($status == 'pending'): ?>
                <div style="text-align: center; margin-top: 15px; padding: 10px; background: #fff3cd; border-radius: 5px;">
                    <small><i class="fas fa-info-circle"></i> Pesanan menunggu approval admin</small>
                </div>
            <?php elseif ($status == 'cancelled'): ?>
                <div style="text-align: center; margin-top: 15px; padding: 10px; background: #f8d7da; border-radius: 5px;">
                    <small><i class="fas fa-times-circle"></i> Pesanan telah dibatalkan</small>
                </div>
            <?php endif; ?>
        </div>
